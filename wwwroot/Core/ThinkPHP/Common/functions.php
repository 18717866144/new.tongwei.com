...
function U($url='',$vars='',$suffix=true,$redirect=false,$domain=false) {
    ...
    if($url) {
        if(0=== strpos($url,'/')) {// 定义路由
			...
			if(C('APP_GROUP_LIST')) { //修改了判断条件，开启子域名部署bug修复部分
			...
			if(C('APP_SUB_DOMAIN_DEPLOY')){
 				foreach (C('APP_SUB_DOMAIN_RULES') as $key => $val) {
					//$ruleGroup = substr($val[0],0,strpos($val[0],'/'));
					//$ruleModel = substr($val[0],strpos($val[0],'/')+1);
					//$ruleAction = substr($val[0],strpos($val[0],'/')+1);
					
					list($ruleGroup,$ruleModel,$ruleAction) = explode('/',$val[0]);
					if(C('URL_CASE_INSENSITIVE')){//检测大小写是否开启
						$ruleGroup = strtolower($ruleGroup);
						$ruleModel = strtolower($ruleModel);
						$ruleAction = strtolower($ruleAction);
					}
					if($var[C('VAR_GROUP')] == $ruleGroup  && $var[C('VAR_MODULE')] == $ruleModel && !$ruleAction){					
						unset($var[C('VAR_GROUP')]);
						unset($var[C('VAR_MODULE')]);
						//unset($var[C('VAR_ACTION')]);
					}
					
					if($ruleAction && $var[C('VAR_ACTION')]==$ruleAction && $var[C('VAR_GROUP')] == $ruleGroup  && $var[C('VAR_MODULE')] == $ruleModel ){					
						unset($var[C('VAR_GROUP')]);
						unset($var[C('VAR_MODULE')]);
						unset($var[C('VAR_ACTION')]);
					}
    ...
    foreach (C('URL_ROUTE_RULES') as $zhaolg => $zlig){//遍历路由
		...
		if(strstr($url,$zlig)){//从网址中找路由规则
            ...
            if($lg){//如果规则带参数                          
				...
				$tempzlg = str_replace('\d','',$tempzlg);//取参数
                //dump($tempzlg);  
                if($tempzlg){
                    if(C('APP_SUB_DOMAIN_DEPLOY')){
