...
class TagLibContent extends TagLibResolve {
	...
	public function _position($attr) {
		...
		$tag['d'] = $tag['d'] ? $tag['d'] : '&gt;';
		$cid = $tag['cid'] ? $tag['cid'] : intval($_GET['cid']);
		$resolve_string = <<<str
		<?php 
		import('ORG.TagLib.Contents');
		\$Contents = new Contents();
		echo \$Contents->getCurrentPosition(\$cid,'{$tag['d']}'); 
		?>
str;
	 ...
	 * 注意：导航解析没有where属性
	 * 
	 * 20140620改进了查询数据库为使用缓存，只有all方法返回玩栏目树，还应优化其他方法，
	 * @param array $attr
	...
	public function _navigate($attr,$content) {
		...
		$tag['limit'] = !isset($tag['limit']) ? $tag['limit'] : 0;//行数	
		$tag['pid'] = !isset($tag['pid']) ? -1 : $tag['pid'];//类型
		$i = 1;
		if (!isset($tag['tag']) || empty($tag['tag'])) $tag['tag'] = 'foreach';//内置TP解析标签
		$tag['shide'] = ( (strtolower($tag['shide'])=='false' || $tag['shide']=='0')) ? false : true;//是否显示隐藏的栏目
		$shide = $tag['is_show']==1 ? 1 :0;//是否显示影藏栏目
		$navigate = F('navigate');
		if ($tag['type']) {
			switch ($tag['type']) {
				case 'all':
					$navigateData = $navigate;                
					foreach($navigateData as $k=>$v){
                        if( ($shide==0 && $v['is_show']!=1) || $v['c_status']!=1 || ($tag['limit'] && $i> $tag['limit']) ) unset($navigateData[$k]);
						else $i++;
					}
                   	$Tree = new Tree();
				...
				case 'son':
					...
					$navigateData = $navigate;
					foreach($navigateData as $k=>$v){
                        if( ($shide==0 && $v['is_show']!=1) || $v['c_status']!=1 || $v['pid']!=$cid || ( $tag['limit'] && $i > $tag['limit'] ) ) unset($navigateData[$k]);
						else $i++;
					}
					break;
				case 'sib':
					...
					$navigateData = $navigate;
					foreach($navigateData as $k=>$v){						
                        if( ($shide==0 && $v['is_show']!=1) || $v['c_status']!=1 || $v['pid']!=$current['pid'] || ( $tag['limit'] && $i > $tag['limit'] ) ) unset($navigateData[$k]);
						else $i++;
					}
					break;
				case 'prt':
                    ...
                    $navigateData = $navigate;
					foreach($navigateData as $k=>$v){
                        if( ($shide==0 && $v['is_show']!=1) || $v['c_status']!=1 || $v['id']!=$current['pid'] || ( $tag['limit'] && $i > $tag['limit'] ) ) unset($navigateData[$k]);
						else $i++;						
					}
					break;
				case 'pid':
					$navigateData = $navigate;
                    foreach($navigateData as $k=>$v){
                        if( ($shide==0 && $v['is_show']!=1) || $v['c_status']!=1 || $v['pid']!=$tag['pid'] || ( $tag['limit'] && $i > $tag['limit'] ) ) unset($navigateData[$k]);
						else $i++;
					}
					break;
			}
