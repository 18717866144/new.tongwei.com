 ...
 ************************************************/  
 ...
 class AutoImages { 
         ...
         function thumbnail($width=128,$height=128, $crop=true, $center=true, $path=null) { 
                 $width = $destw  = min($this->getWidth(), $width); 
                 $height = $desth = min($this->getHeight(), $height); 
                 //高度随比例自动
                 if(empty($height)||$height==0) {
                 	$oldWidth = $this->getWidth();
                 	$ratio =  $width/$oldWidth;
                 	$desth = $height = round($this->getHeight() * $ratio);
                 }
                 if($crop){ 
                         $srcw = $this->getWidth(); 
                         $srch = $this->getHeight();                      
         ...
         /*******************************自动在线输出***********************************/
         static public function autoOutput($file_path,$width,$height) {
	         	self::$AutoIMG = empty(self::$AutoIMG) ? new AutoImages() : self::$AutoIMG;
	         	self::$AutoIMG->loadFile($file_path)->thumbnail($width,$height)->output("jpg");
         }
} 
 ?>