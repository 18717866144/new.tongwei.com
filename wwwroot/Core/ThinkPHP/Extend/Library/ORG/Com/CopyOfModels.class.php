...
class Models {	
	...
	public function getDisplay($model,$type,$data = array(),$action_mark ='') {
		...
		//注意两边参数以及两边参数顺序，都不一样
		return $model['model_type']=='member' ? $this->getMemberDisplay($modelField,$type,$action_mark,$data) : $this->getContentDisplay($modelField,$type,$data);
	}
	...
	private function getMemberDisplay($modelField,$type,$model_mark,array $data) {
		...
		foreach ($modelAction as $modelValues) {
			...
			$newModelField['html'] = $modelValues[1] == 1 ? str_replace('ignore="ignore"', '', $newModelField['html']) : preg_replace('(\/?\>)', 'ignore="ignore" />', $newModelField['html']);
	 ...
	 * @return Ambigous <multitype:, unknown>
	 */
	public function getAddDisplay($model,$action_mark) {
		$mid = $model['id'];
		$modelField = F("models_fields_$mid");
		uasort($modelField, create_function('$a,$b', 'return $a["sort"] < $b["sort"];'));//if($a["sort"] > $b["sort"]) {return 1;} elseif($a["id"] > $b["id"]) {return 1;} else {return -1;}
		require_once REQUIRE_PATH.'Fields/Field.class.php';
		return $model['model_type']=='member' ? $this->getMemberAddDisplay($modelField,$action_mark) : $this->getContentAddDisplay($modelField);
	}
	 ...
	 * @return Ambigous <multitype:, unknown>
	 */
	private function getContentAddDisplay($modelField) {
		$fieldArray = array();
		foreach ($modelField as $model_key=>$model_values) {
			/* 去除非前台投搞字段和内置字段 */
			if (SESSION_TYPE != 1 && ($model_values['field_setting']['is_posted'] == 2 || $model_values['field_setting']['is_internal'] == 1)) continue;
			...
			$fieldsObject->setting = $model_values['field_setting'];
			$fieldsObject->type = 1;
			$model_values['html'] = $fieldsObject->show();
	 ...
	 * @return Ambigous <multitype:, number>
	 */
	private function getMemberAddDisplay($modelField,$model_mark) {
		$modelAction = M('ModelAction')->where("name='$model_mark'")->limit(1)->getField('fields');
		$modelAction = explode(',', trim($modelAction,','));
		$newModelField = array();
		$fieldArray = array();
		foreach ($modelAction as $modelValues) {
			...
			$fieldsObject->setting = $newModelField['field_setting'];
			$fieldsObject->type = 1;
			$newModelField['html'] = $fieldsObject->show();
			$newModelField['html'] = $modelValues[1] == 1 ? str_replace('ignore="ignore"', '', $newModelField['html']) : preg_replace('(\/?\>)', 'ignore="ignore" />', $newModelField['html']);
			$newModelField['field_setting']['is_basic'] == 1 ? $fieldArray['is_basic'][] = $newModelField : $fieldArray['is_append'][] = $newModelField;
		}
		return $fieldArray;
	 ...
	 * @param numeric $mid
	 * @param array $data
	 * @return Ambigous <multitype:, unknown, mixed, number, boolean, string>
	 */
	public function getEditDisplay($mid,$data) {
		...
		foreach ($modelField as $model_key=>$model_values) {
			...
			$fieldsObject->setting = $model_values['field_setting'];
			$fieldsObject->type = 2;
			$model_values['html'] = $fieldsObject->show($data[$model_key]);
