...
<?php if (isset($setting)) { ?>
     ...
     <tr>
        ...
        <td align="left">
         	...
         	<input type="radio" name="setting[allow_type]" value="soft" <?php echo $setting['allow_type']=='soft' ? 'checked="checked"' : ''?> onclick="$('.soft_setting').show().siblings().hide();$('#small_file_size_type').attr('checked',true);$('#file_size_type').hide();"> 软件 
	        <input type="radio" name="setting[allow_type]" value="media" <?php echo $setting['allow_type']=='media' ? 'checked="checked"' : ''?> onclick="$('.media_setting').show().siblings().hide();$('#file_size_type').show();"> 媒体
            </td>
		</tr>
		
		<?php if($setting['allow_type']=='media'){?>
		<tr id="file_size_type">
			<td width="120">文件大小类型</td>
			<td align="left">
				<input type="radio" name="setting[file_size_type]" <?php echo $setting['file_size_type']=='large' ? 'checked="checked"' : ''?> value="large"> 大文件
				<input id="small_file_size_type" type="radio" name="setting[file_size_type]" <?php echo $setting['file_size_type']=='small' ? 'checked="checked"' : ''?> value="small"> 小文件
			</td>
		</tr>
		<?php }?>
       ...
       <div class="img_setting" style="<?php echo $setting['allow_type']=='img' ? '' : 'display: none;'?>">
	    <input type="text" name="setting[allow_image_type]" disabled="disabled" readonly value="<?php echo implode('|', C('ALLOW_IMAGE_TYPE'))?>" class="text">修改类型请到"上传配置"中修改</div>
        <div class="soft_setting" style="<?php echo $setting['allow_type']=='soft' ? '' : 'display: none;'?>">
	    <input type="text" name="setting[allow_soft_type]" disabled="disabled" readonly value="<?php echo implode('|', C('ALLOW_SOFT_TYPE'))?>" class="text">修改类型请到"上传配置"中修改</div>
        <div class="media_setting" style="<?php echo $setting['allow_type']=='media' ? '' : 'display: none;'?>">
        <input type="text" name="setting[allow_media_type]" disabled="disabled" readonly value="<?php echo implode('|', C('ALLOW_MEDIA_TYPE'))?>" class="text">修改类型请到"上传配置"中修改</div>
            </td>
			</tr>
	    	<tr>
...
<table>
        ...
        <td width="120">文件类型</td>
        <td align="left">
        <input type="radio" name="setting[allow_type]" value="img" onclick="$('.img_setting').show().siblings().hide();$('#file_size_type').hide();"> 图片 
        <input type="radio" name="setting[allow_type]" value="soft" onclick="$('.soft_setting').show().siblings().hide();$('#file_size_type').hide();"> 软件 
	        <input type="radio" name="setting[allow_type]" value="media" onclick="$('.media_setting').show().siblings().hide();$('#file_size_type').show();"> 媒体
            </td>
		</tr>
        <tr id="file_size_type">
			<td width="120">文件大小类型</td>
			<td align="left">
				<input type="radio" name="setting[file_size_type]" value="large"> 大文件
				<input type="radio" name="setting[file_size_type]" checked="checked" value="small"> 小文件
			</td>
		</tr>
	    <tr> 
        <td width="120">允许类型</td>
		<td>
        <div class="img_setting" style="<?php echo $setting['allow_type']=='img' ? '' : 'display: none;'?>">
	    <input type="text" name="setting[allow_img_type]" disabled="disabled" readonly value="<?php echo implode('|', C('ALLOW_IMAGE_TYPE'))?>" class="text">修改类型请到"上传配置"中修改</div>
	    <div class="soft_setting" style="<?php echo $setting['allow_type']=='soft' ? '' : 'display: none;'?>">
	    <input type="text" name="setting[allow_soft_type]" disabled="disabled" readonly value="<?php echo implode('|', C('ALLOW_SOFT_TYPE'))?>" class="text">修改类型请到"上传配置"中修改</div>
        <div class="media_setting" style="<?php echo $setting['allow_type']=='media' ? '' : 'display: none;'?>">
        <input type="text" name="setting[allow_media_type]" disabled="disabled" readonly value="<?php echo implode('|', C('ALLOW_MEDIA_TYPE'))?>" class="text">修改类型请到"上传配置"中修改</div>
