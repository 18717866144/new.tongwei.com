...
<div class="video-b" >
    ...
    <div class="video-bd" >
		...
		<sql sql="select * from h_special_content where a_status=1 and typeid=5 and specialid={$specialid} order by sort desc,id desc limit 10">
...
<div class="index-1 clearfix">
	...
	<div class="left">
		...
		<sql sql="select * from h_special_content where a_status=1 and typeid=1 and specialid={$specialid} and id not in({$idsStr}) order by sort desc,id desc limit 5">
		<li><span class="date">{-:date('Y-m-d',$values['save_time'])-}</span><a href="{-$values['url']-}" target="_blank">{-$values.title-}</a></li>
		</sql>
		</ul>
		
		<div id="history" class="title-wrap" style="margin-top:15px"><span class="more"><a href="{-:U('modules/special/lists?specialid='.$specialid.'&typeid=6')-}" target="_blank">更多</a></span><h2><a href="{-:U('modules/special/lists?specialid='.$specialid.'&typeid=5')-}" target="_blank">往期回顾</a></h2></div>
		<ul class="index-news">
		<sql sql="select * from h_special_content where a_status=1 and typeid=6 and specialid={$specialid} order by sort desc,id desc limit 6">
		<li><span class="date">{-:date('Y-m-d',$values['save_time'])-}</span><a href="{-$values['url']-}" target="_blank">{-$values.title-}</a></li>
		</sql>
		</ul>
	</div>
