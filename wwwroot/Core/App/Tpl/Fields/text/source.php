...
<input type="text" size="30" name="info[<?php echo $this->form['field_name']?>]" id="source" value="<?php echo $value['source'];?>" <?php echo $this->form['input_attr']?> />
...
<div id="source_div" show_status="show" style="display:none;position: absolute;bottom:25px;background:#F0F8FD;left:8px;width:200px;z-index:200;padding:10px;border:1px solid #95AADB">
	<?php 
		...
		foreach ($source as $v) {
	?>
	<a href="javascript:;" style="padding:3px 7px; " onclick="$('#source').val($(this).text());$('#source_div').attr('show_status','show');$('#source_div').hide();"><?php echo $v?></a>
	<?php }?>
</div>
</div>
