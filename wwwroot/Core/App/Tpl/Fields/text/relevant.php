...
<?php if ($value['relevant_title']) { foreach ( $value['relevant_title'] as $reValue) {?>
	<p><?php echo String::msubstr($reValue['title'], 0,13)?><a href="javascript:;" class="setRed FR" style="margin-right:5px;" id="<?php echo $reValue['id']?>">╳</a></p>
<?php } }?>
</div>
<div style="text-align:center;">
	<input type="button" value="添加相关" class="smallSub" onclick="openNewWindow('<?php echo U('Content/Contents/relevant',array('cid'=>$_GET['cid'],'file_name'=>"info[{$this->form['field_name']}]"))?>','相关文章',600,400)" />
