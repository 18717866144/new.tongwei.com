	...
	<div class="content_form clearfix">
		...
		<?php foreach ($newFieldArray as $values) {?>
			<tr <?php if ($values['field_setting']['is_hide'] == 1) {echo 'style="display:none;"';}?>>
				<td class="td_left" style="width:85px;">
		...
		<?php if ($start_code == 1) {?>
			<tr>
				<td class="td_left" style="width: 85px;">验证码</td>
				<td class="td_right"><input type="text" name="code" class="code" /><img src="__URL__/get_code?w=120&h=38&l=5&fs=16&mid=<?php echo $_GET['mid']?>" onclick="this.src=this.src+'&_='+Math.random()" alt="" /></td>
			</tr>
		<?php }?>
			<tr>
				<td class="td_left">&nbsp;&nbsp;</td>
