...
<table class="showTable">
	<tr>
		...
		<tr>
			...
			<td><?php echo date('Y-m-d H:i:s',$values['login_time'])?></td>
		</tr>
		<?php }?>
		<tr>
			<td colspan="5"><?php echo $logData[1]?></td>
		</tr>
</table>
<div class="btns">
	<input type="button" value="反选" id="invert_selection" class="sub" />
	<?php if (in_array('delete', $ruleOther)) {?>
		<input type="button" value="删除" onclick="_confirm('是否确定要删除？',function(){$.CR.G.bulkAction('<?php echo U('delete',array('type'=>'login_success'))?>')});" class="sub" />
	<?php }?>
</div>
