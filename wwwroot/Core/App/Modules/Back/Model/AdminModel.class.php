...
// 				非超管 
				if (!isset($userData['admin_group'])) $userData['admin_group'] = $userGroup;
	 ...
	 * @return boolean
	 */
	public function setDecAdminMoney($id,$money,$update_session = false) {
		$status = $this->where("id=$id")->setDec('money', $money);
		if ($status === false) {
			$this->writeLog("扣除管理员金额失败，用户ID{$id}", 'SYSTEM_ERROR');
			return false;
		} else {
			if (GBehavior::$session && $update_session) {
				GBehavior::$session['money'] = $_SESSION[C('ADMIN_SESSION')]['money'] = $_SESSION[C('ADMIN_SESSION')]['money']-$money;
			}
			return true;
		}
	}
