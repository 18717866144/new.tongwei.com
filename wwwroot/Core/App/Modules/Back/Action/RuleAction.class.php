...
class RuleAction extends GlobalAction {
	...
	public function index() {
		$topid = i('topid',0,'intval');
		$map = array();
		if($topid){
			//$map['pid'] = $topid;	
		}
		$nodeData = $this->model->where($map)->order('sort DESC')->select();
		$Tree = new Tree();
		$Tree->icon = array('&nbsp;&nbsp;&nbsp;│  ', '&nbsp;&nbsp;&nbsp;├─  ', '&nbsp;&nbsp;&nbsp;└─  ');
		$Tree->nbsp = '&nbsp;&nbsp;&nbsp;';
		foreach ($nodeData as &$values) {
			$values['str_manage'] = '<a href="'.U('add',array('id'=>$values['id'])).'" class="operate">添加子菜单</a>&nbsp;&nbsp;<a class="operate" href="'.U('edit',array('id'=>$values['id'])).'">编辑</a>&nbsp;&nbsp;<a class="operate" onclick="(_confirm(\'是否确定要删除？\',function(){location.href=\''.U('delete',array('id'=>$values['id'])).'\'}))" href="javascript:;">删除</a>';
		...
		$Tree->init($nodeData);
		//$this->assign('nodetreeData',$Tree->get_tree_array(0));
		$str = "<tr>
