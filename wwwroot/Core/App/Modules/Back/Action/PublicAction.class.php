...
class PublicAction extends GlobalAction {
	...
	//验证Api是否重复
	public function check_api_set() {
		if (!IS_AJAX) $this->error(C('NOT_AJAX'));
		$name = $this->checkData('param');
		$id = $this->checkData('id','GET',false);
		$where = $id ? "AND id<>'$id'" : '';
		$Model = M('ApiSet');
		$data = $Model->where("name='$name' $where")->find();
		$data ? $this->ajaxReturn('','此Api标识已存在！','n') : $this->ajaxReturn('','','y');
	}
	...
	//生成地区JSON数据
	public function _regions(){
		$region = M('region');
		$data = $region->getField('id,pid,name,typeid');
		F('region',$data);
		$array = array();
		foreach($data as $k=>&$v){
			$array[$v['pid']][] = &$v;	
		}
		$str = 'var region='.json_encode($array).';';
		$status = file_put_contents(__PATH__.'Public/js/region.js',$str);
		return $status;
	}
	...
	public function _navigate() {
		$navigate = M('Navigate')->order('sort desc,id asc')->select();
		$newNavigate = array();
	...
	public function _models() {
		...
		//Models Fields 模型字段
		$fields = M('ModelsFields')->where('field_status=1')->order(' sort desc,id asc ')->select();
		$newFields = array();
	...
	private function _updateMyApp($DESKTOP,$AdminDesktop) {
		...
		public function getFiles(){
			$base = './Uploads';
			$dir = I('get.dir');
			$dir = isset($dir)?urldecode($dir):'';			
			$root = empty($dir) ? $base : $base . $dir;
			$d = scandir($root);
			$list = array();
			foreach($d as $v) {
				$file = rtrim($root,'/').'/'.$v;
				...
				$charset = mb_detect_encoding($fileName, array('UTF-8','GBK','GB2312')); 
				...
				$filePath = $dir.'/'.urlencode($fileName);
				...
				$a['filePath'] = $filePath;
				$a['fileType'] = $fileType;
				$a['fileExt'] = pathinfo($file, PATHINFO_EXTENSION);					
				$a['fileAtime'] = $b['atime'];
				$a['fileMtime'] = $b['mtime'];
				$a['fileCtime'] = $b['ctime'];
				$a['fileSize'] = byte_format($b['size']);
