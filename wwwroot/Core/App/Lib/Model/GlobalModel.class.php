...
// 				时间设置==>没有设定时间，默认为拿出今天
				if ($isToday && empty($_GET[$start_key]) && empty($_GET[$end_key])) {
					...
					$where[$field] = array('BETWEEN',array($start_time,$end_time));
					//$where .= " AND $field BETWEEN $start_time AND $end_time";
				} else {
					//$where[$field] = array('BETWEEN',array(strtotime($_GET[$start_key]),strtotime($_GET[$end_key])));
		...
		if (isset($_GET[$start_key]) && !empty($_GET[$start_key]) && empty($_GET[$end_key]) ) {
			$where[$field] = array('egt',$start_type);
			//$where .= " AND $field >=$start_type";
			$url .= "&$start_key={$_GET[$start_key]}";
		}
		if (isset($_GET[$end_key]) && !empty($_GET[$end_key])  && empty($_GET[$start_key])  ) {
			$where[$field] = array('elt',$end_type);
			//$where .= " AND $field <= $end_type";
			$url .= "&$end_key={$_GET[$end_key]}";
		}
		
		if (!empty($_GET[$end_key])  && !empty($_GET[$start_key])  ) {
			$where[$field] = array('BETWEEN',array(strtotime($_GET[$start_key]),strtotime($_GET[$end_key])));
			//$where .= " AND $field <= $end_type";
			$url .= "&$end_key={$_GET[$end_key]}";
		}		
	 ...
	 * @return array
	 */
	protected function resolveGET($array,$condition = 'eq') {
		$result = array();
		$where = array();
		foreach ($array as $key=>$value) {
			...
			if (isset($_GET[$value]) && $_GET[$value] != '') {
				...
				$where[$key] = array($condition,$get_value);
				$url = "&$key=$get_value";
				$result['url'] .= $url;
			}
		}
