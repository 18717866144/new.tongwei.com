...
//				parent.$('#thumbnail_<?php echo $file_name;?> img').attr({src:'__ROOT__/'+_path,alt:_path});
	...
	//	uploadLimit : '<?php ?>//',//一次最多只允许上传10条
	//	queueID:'fileQueue',//上传框显示的id
	//	buttonCursor: 'pointer',
	//	swf      : '__PUBLIC__/js/uploadify/uploadify.swf',
	//	uploader : '__URL__/public_upload',
	//	method   : 'post',//方法，服务端可以用$_POST数组获取数据
	//	formData:{up_type:'img',related:'<?php ?>//',ext:'<?php ?>//',SESSION_ID:'<?php ?>//'},
	//	fileTypeDesc : '允许上传的格式为*.<?php ?>//;',//允许上传的格式
	//	fileTypeExts : '*.<?php ?>//;',//限制允许上传的图片后缀*.jpg;*.gif;
	//	fileSizeLimit : '<?php ?>//',//限制上传的图片不得超过200KB
	...
	$('div.showList').on('click','a',function(){
		...
		//判断选择是否超过最大附件数
		if(_thisNum > <?php echo ($_GET['up_num']); ?>) {_alert('最多只允许上传<?php echo ($_GET['up_num']); ?>个附件！！！');}
