...
<div class="menuwrap">
    ...
    <div class="fw2">
        ...
        <div class="aboutbox pt30">
            <div class="fw2">
                <!--<p class="intitle"><?php echo ($navigate['navigate_name']); ?></p>-->
                <ul class="news-center-list">
                    		<?php
 import('ORG.TagLib.Contents'); $contentClass = new Contents(); $contentClass->tags = array ( 'type' => 'page', 'cid' => ''.($cid).'', 'order' => 'save_time desc', 'cache' => false, 'limit' => '10', 'field' => 'id,cid,title,save_time,style,click,url,description,thumbnail', 'return' => 'data|values', 'tag' => 'foreach', 'tbdt' => true, 'tbpx' => 'small_', ); $data = $contentClass->_page(); $i=1; if(is_array($data['page'])): foreach($data['page'] as $key=>$values): if(!$values['thumbnail']){ $cmodel = new Model(); $sql="select * from h_article_data where aid =".$values['id']; $con=$cmodel->query($sql); $pattern ="/<img.*?src=[”|’]?(.*?)[”|’]?s.*?>/i"; $content=$con[0]["content"]; $a=preg_match_all($pattern,$content,$match); $imgSrcArr=[]; if (isset($match[0])){ foreach ($match[0] as $key => $imgTag){ $pattern_src = "/src=.*[gif|jpg|jpeg|png]\"/i"; preg_match_all($pattern_src,$imgTag,$matchSrc); if (isset($matchSrc[0])){ foreach ($matchSrc[0] as $src){ $src=str_replace('src="','',$src); $src=str_replace('"','',$src); $imgSrcArr[] = $src; } } } } $values['thumbnail']=$imgSrcArr[0]; } ?>
                    <li aos="fade-up">
                        <a href="<?php echo ($values['url']); ?>">
                            <div class="pic">
                                <div class="img">
                                    <img src="<?php if(!empty($values["thumbnail"])): echo ($values["thumbnail"]); else: ?>__PUBLIC__/assets/images/default.png<?php endif; ?>" alt="" height="100%" width="100%">
                                </div>
                            </div>
                            <div class="con">
                                <div class="ntop">
                                    <p class="tt"><?php echo ($values["a_title"]); ?></p>
                                    <div class="desc"><?php echo trim($values['description'],'　');?>
                                    </div>
                                </div>
                                <p class="date"><?php echo date('Y-m-d',$values['save_time']);?> <?php echo ($values['save_time']); ?></p>
                            </div>
                        </a>
                    </li><?php $i++; endforeach; endif; $page=$data['pageinfo'];?>
                </ul>
                <div class="page-pt">
                    <?php echo ($page); ?>
                </div>
