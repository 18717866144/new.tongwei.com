	...
	<tr>
		...
		<tr>
			...
			<td>
				...
				<?php }?>
				<?php  if ($values['attr']) { $attrString = '<span class="setRed">['; $attr = explode(',', $values['attr']); foreach ($attr as $value) { switch ($value) { case 'h': $attrString .='头条新闻&nbsp;'; break; case 'd': $attrString .='首页不显示&nbsp;'; break; case 'f': $attrString .= '推荐&nbsp;'; break; } } $attrString = rtrim($attrString,'&nbsp;'); $attrString .= ']</span>'; } else { $attrString = ''; } echo $attrString; ?>
			</td>
			...
			<td>
					...
					<?php }else { ?>
					<a href="<?php echo $values['url'];?>" target="_blank" class="operate">查看</a>&nbsp;&nbsp;
					<?php } ?>
