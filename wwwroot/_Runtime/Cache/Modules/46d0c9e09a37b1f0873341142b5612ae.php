						...
						<div class="banner_farm">
							<div class="banner_pos">
								...
								<div class="banner_con">
                                	<ul>
												<?php
 import('ORG.TagLib.Resolve'); $contentClass = new Resolve(); $contentClass->tags = array ( 'sql' => 'select * from h_special_content where FIND_IN_SET(\'h\',attr) and a_status=1 and typeid=3 and specialid='.($specialid).' order by id desc,sort desc limit 5', 'type' => 'list', 'return' => 'data|values', 'tag' => 'foreach', 'cache' => false, ); $data = $contentClass->_sql(); $i=1; if(is_array($data)): foreach($data as $key=>$values): ?><li><a href="<?php echo ($values['url']); ?>" target="_blank"><img src="<?php echo thumb($values['thumbnail'],1000,420);?>" /></a></li><?php $i++; endforeach; endif; ;?>						
                                    </ul>
                                 </div>              
 ...
 import('ORG.TagLib.Resolve'); $contentClass = new Resolve(); $contentClass->tags = array ( 'sql' => 'select * from h_special_content where a_status=1 and typeid=1 and specialid='.($specialid).' order by id desc,sort desc limit 5', 'type' => 'list', 'return' => 'data|values', 'tag' => 'foreach', 'cache' => false, ); $data = $contentClass->_sql(); $i=1; if(is_array($data)): foreach($data as $key=>$values): ?><li><a href="<?php echo ($values['url']); ?>" target="_blank"><span class="fnews_tag news_one"><?php echo sprintf("%02d",$i);?></span><span class="fnews_txt"><?php echo ($values['title']); ?></span></a></li><?php $i++; endforeach; endif; ;?>
	...
	<div class="media_wrap clearfix">
		...
		<div class="wap_tit">独家报道</div>
			...
			<ul>
					<?php
 import('ORG.TagLib.Resolve'); $contentClass = new Resolve(); $contentClass->tags = array ( 'sql' => 'select * from h_special_content where FIND_IN_SET(\'j\',attr) and a_status=1 and typeid=3 and specialid='.($specialid).' order by id desc,sort desc limit 5', 'type' => 'list', 'return' => 'data|values', 'tag' => 'foreach', 'cache' => false, ); $data = $contentClass->_sql(); $i=1; if(is_array($data)): foreach($data as $key=>$values): ?><li><a href="<?php echo ($values['url']); ?>" target="_blank"><img src="<?php echo thumb($values['thumbnail'],300,300,1);?>" alt="<?php echo ($values['title']); ?>"><span><em><?php echo ($values['title']); ?></em></span></a></li><?php $i++; endforeach; endif; ;?>					
			</ul>
			</div>
		...
		<div class="news_hz">新闻汇总<a href="<?php echo U('modules/special/lists',array('specialid'=>$specialid,'typeid'=>3));?>">更多>></a></div>
			<div class="news_li">
				<ul>
							<?php
 import('ORG.TagLib.Resolve'); $contentClass = new Resolve(); $contentClass->tags = array ( 'sql' => 'select * from h_special_content where a_status=1 and typeid=3 and specialid='.($specialid).' order by id desc,sort desc limit 9', 'type' => 'list', 'return' => 'data|values', 'tag' => 'foreach', 'cache' => false, ); $data = $contentClass->_sql(); $i=1; if(is_array($data)): foreach($data as $key=>$values): ?><li><a href="<?php echo ($values['url']); ?>"  target="_blank"><?php echo ($values['title']); ?></a><div class="date"><i><?php echo ($values['source']); ?></i><span><?php echo date('Y-m-d',$values['save_time']);?></span></div></li><?php $i++; endforeach; endif; ;?>
				</ul>        
			</div>
		</div>
    </div>
