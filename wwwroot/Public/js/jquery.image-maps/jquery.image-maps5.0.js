			...
			$.fn.imageMaps = function(setting){
				...
				function initAddClickEvent(){
					...
					$("#add_hot_area").unbind("click").click(function () {
						...
						$('#areaItems').append(createAreaItem(index,'','','','0,0,90,30',setting.tag,setting.domCallBack(setting.params)));
						if(initFlag){
				...
				function initDeleteClickEvent(){
					...
					$('.hot_area .delete').unbind('click').click(function () {
						...
						updateAreaCount();
						var contentId = $(this).parents('[ref]').eq(0).find('.contentId').val();
						contentId  = contentId || -1
						setting.contentDelete(ref,contentId);
					});
					
					//初始化添加文章按钮 
					$('.hot_area .areaActionAdd').unbind('click').click(function () {
						var ref = $(this).parents('[ref]').eq(0).attr('ref');						
						/*var areaContainer = $(".hot_area");*/
						setting.contentAdd(ref);
					});
					
					//初始化修改文章按钮 
					/*
					$('.hot_area .areaActionEdit').unbind('click').click(function () {
						
					});
					*/
					$('#areaItems').delegate('.areaActionEdit', 'click', function(event){
						var ref = $(this).parents('[ref]').eq(0).attr('ref');
						var contentId = $(this).parents('[ref]').eq(0).find('.contentId').val();						
						setting.contentEdit(ref,contentId);
						event.stopPropagation();
					});
					
					//初始化添加文章按钮 
					$('.hot_area .areaActionSelect').unbind('click').click(function () {
						var ref = $(this).parents('[ref]').eq(0).attr('ref');
						setting.contentSelect(ref);
						/*
						var areaContainer = $(".hot_area");	
						art.dialog({
							title: '文件管理',
							...
							opacity:0,
							padding:0,
							content: 'setting.contentSelectUrl + ref',
							time: 0
					...
					if ($('.hot_area img[ref=imageMap]').attr("src")
						...
						if (areaInfoJson != null && areaInfoJson.length > 0) {
							for (var j = 0; j < areaInfoJson.length; j++) {
								...
								var contentId = areaInfoJson[j].contentId;								
								for(var attr in setting.params){
								...
								if(areaMapInfo) {
	                            	...
	                            	$('#areaItems').append(createAreaItem(index, areaTitle, areaLink, contentId, areaMapInfo,setting.tag,setting.domCallBack(setting.params)));
	            					if(initFlag){
	            						setting.initCallBack(setting.params);
	            					}
				...
				//done
				function createAreaItem(index, areaTitle, areaLink, contentId, areaMapInfo,rowTagDom,specialHotItemDom) {
//console.log("specialHotItemDom:"+specialHotItemDom);
					...
					if (rowTagDom == "tr") {
						...
						item.push('<label for="">标题：</label><input type="text" value="'+ c+ '" class="w45 ipt_border area_title"/>');
						item.push(specialHotItemDom);
					...
					*/
						...
						item.push('<span class="areaSort">热区' + index + '：</span>');
						item.push('<input type="hidden" name="areaMapInfo[]" class="areaMapInfo" value="'+ areaMapInfo + '" />');
						item.push('<input type="hidden" name="areaTitle[]" class="areaTitle" value="'+ areaTitle + '" />');
						item.push('<input type="hidden" name="areaLink[]" class="areaLink" value="'+ areaLink + '" />');
						item.push('<input type="hidden" name="contentId[]" class="contentId" value="'+ contentId + '" />');
						item.push('<span class="areaTitleHtml '+(areaTitle?'':'no')+'">'+(areaTitle?'<a href="'+areaLink+'" target="_blank">'+areaTitle+'</a>':'还未关联文章')+'</span>');						
						item.push('<span style="float:right">');
						
						if(contentId){
							item.push('<a href="javascript:;" class="second_btn ml areaActionEdit sub" title="修改文章">修改</a> ');
						}else{
							item.push('<a href="javascript:;" class="second_btn ml areaActionAdd sub" title="添加新文章">添加</a> ');
						}						
						
						item.push('<a href="javascript:;" class="second_btn ml areaActionSelect sub" title="添加的文章未成功关联到版面时，选择已添加文章">选择</a> ');
						item.push('<a href="javascript:;" class="second_btn ml delete sub">删除</a>');
						item.push('</span>');
						item.push('</li>');
					/* } */
					return item.join('');
