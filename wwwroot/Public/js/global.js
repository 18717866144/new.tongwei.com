...
(function(window,$){
	$.G = {
		...
		Tabs:function(index){
			var $tabs = $('.tabs'),$tabsTitle = $('.tabs').find('.tabs-title'),$tabsContent=$('.tabs').find('.tabs-content'),i=0;
			index = index?index:0;
			$tabsTitle.children().click(function(){
				i = $(this).index();
				$tabsTitle.children().removeClass('hover');
				$(this).addClass('hover');
				$tabsContent.children().removeClass('hover').hide();
				$tabsContent.children().eq(i).show();
			});			
			$tabsTitle.children().eq(index).click();
		}
	};
})(window,jQuery);
/****jQuery自身扩展*****/
...
function _tips(data,isreload,promptType) {
	...
	if(promptType == 'tips') {
		...
		if((data.status == 'success' || data.status == 1) && isreload) setTimeout(function(){location.reload()},800);
	} else {
		_alert(_info);
	}
}
